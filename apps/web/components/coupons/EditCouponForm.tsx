...
import { updateCoupon } from '@/lib/coupons/coupons'
import { toast } from 'sonner'
import { Loader2 } from 'lucide-react'
import { AxiosError } from 'axios'
import { useRouter } from 'next/navigation'

export function EditCouponForm({ initialData }: { initialData: any }) {
  const router = useRouter()
  const formattedExpiresAt = initialData.expires_at
    ? new Date(initialData.expires_at).toISOString().slice(0, 16)
    : ''
  const [code, setCode] = useState(initialData.code)
  const [type, setType] = useState(initialData.type)
  const [amount, setAmount] = useState(initialData.amount)
  const [expiresAt, setExpiresAt] = useState(formattedExpiresAt)
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      setLoading(true)
      const response = await updateCoupon(initialData.id, {
        code,
        type,
        amount: amount,
