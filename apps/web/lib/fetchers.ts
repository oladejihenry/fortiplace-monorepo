...
export async function getProductsByDomain(domain: string, options: FetchOptions = {}) {
  ...
  try {
    ...
    const response = await axios.get(
      `/api/store/show/${encodeURIComponent(cleanDomain || '')}/products`,
      {
        headers: {
          Accept: 'application/json',
        },
...
export async function getProductBySlug(
  ...
  try {
    ...
    const cleanDomain = domain.split(':')[0]
    const userAgent = customHeaders?.['user-agent'] || customHeaders?.['User-Agent']

    const response = await axios.get(
      `/api/store/show/${encodeURIComponent(cleanDomain || '')}/products/${slug}`,
      {
...
export async function getSiteUserByDomain(domain: string) {
  try {
    ...
    const response = await axios.get(
      `/api/store/show/${encodeURIComponent(cleanDomain || '')}/user`,
      {
        headers: {
          Accept: 'application/json',
        },
